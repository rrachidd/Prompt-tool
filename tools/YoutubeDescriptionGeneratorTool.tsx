...
const YoutubeDescriptionGeneratorTool: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const result = await generateYoutubeDescription(title);
            setGeneratedDescription(result);
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
            setError(`حدث خطأ أثناء إنشاء الوصف: ${errorMessage}`);
            console.error(err);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="h-full flex flex-col">
            <h2 className="text-3xl font-bold text-white mb-2">توليد وصف لفيديو يوتيوب</h2>
            <p className="text-brand-light mb-6">أنشئ وصفًا كاملاً ومُحسَّنًا لمحركات البحث (SEO) لفيديو يوتيوب الخاص بك.</p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-4">
                <input
                    ...
                    placeholder="أدخل عنوان الفيديو هنا..."
                ...
                >
                    {isLoading ? '...جاري الإنشاء' : 'أنشئ الوصف'}
                </button>
            </div>
            ...
            <div className="flex-grow bg-brand-dark border border-brand-mid rounded-lg p-4 mt-4 relative min-h-[300px] flex flex-col">
                ...
                {!isLoading && generatedDescription && (
                    <>
                        <div id="printable-area" className="flex-grow">
                             <pre className="whitespace-pre-wrap text-brand-extralight w-full h-full font-cairo text-right">{generatedDescription}</pre>
                        </div>
                ...
                {!isLoading && !generatedDescription && !error && (
                    <p className="text-brand-light m-auto">سيظهر وصف الفيديو الاحترافي هنا.</p>
                )}
            </div>
        </div>
    );
